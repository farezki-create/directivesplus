...
export const useRegisterWithAwsEmail = () => {
  const [isLoading, setIsLoading] = useState(false);
  const { sendEmail } = useAwsEmail();

  const register = async (values: z.infer<typeof registerFormSchema>) => {
    setIsLoading(true);
    
    try {
      console.log("🚀 Début du processus d'inscription avec AWS SES pour:", values.email);
      ...
      // Email non confirmé, envoyer un email personnalisé via AWS SES
      if (data.user && !data.user.email_confirmed_at) {
        console.log("📧 Envoi d'email de confirmation personnalisé via AWS SES");
        ...
        // Envoyer l'email de confirmation personnalisé via AWS SES
        ...
        console.log("📬 Résultat envoi email AWS SES:", emailResult);
        
        if (emailResult.success) {
          toast({
            title: "Inscription réussie !",
            description: "Un email de confirmation a été envoyé à votre adresse via AWS SES. Veuillez cliquer sur le lien pour activer votre compte.",
            duration: 10000
          });
          ...
          return { 
            ...
            message: "Email de confirmation AWS SES envoyé"
          };
        } else {
          console.warn("⚠️ Échec de l'envoi d'email personnalisé AWS SES");
