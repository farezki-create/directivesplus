...
import { FileText, Eye, Trash2, FileIcon, Image, CreditCard } from "lucide-react";
...
export function MedicalDocumentsList({
  ...
  const getDocumentIcon = (fileType: string, description: string) => {
    if (description?.includes("carte d'accès") || description?.includes("Carte d'accès")) {
      return <CreditCard className="h-5 w-5 text-purple-500 mr-2" />;
    }
  ...
  return (
    <div className="space-y-4">
      {documents.map((document) => {
        const isCard = document.description?.toLowerCase().includes("carte d'accès");
        return (
          <Card key={document.id} className={`overflow-hidden ${isCard ? 'border-purple-200' : ''}`}>
            <div className="p-4">
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center">
                  {getDocumentIcon(document.file_type, document.description)}
                  <span className={`font-medium ${isCard ? 'text-purple-700' : ''}`}>
                    {document.file_name}
                  </span>
                </div>
              </div>
