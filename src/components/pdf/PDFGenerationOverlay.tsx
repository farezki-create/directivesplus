...
export function PDFGenerationOverlay({ 
  ...
  const [iconAnimation, setIconAnimation] = useState("animate-pulse");
  ...
  useEffect(() => {
    if (isGenerating) {
      ...
      const iconAnimations = ["animate-pulse", "animate-bounce", "animate-spin"];
  ...
  return (
    <div className={`border rounded-lg p-6 mb-6 ${bgGradient} space-y-4 animate-fade-in`}>
      ...
      <Progress 
        value={progress} 
        className={`h-2 bg-${accentColor}-100`} 
      />
      
      <div className="flex items-center gap-2 text-sm">
        <Loader2 className={`h-4 w-4 animate-spin ${iconColor}`} />
        <span className={`text-${accentColor}-700 ${animationClass}`}>
      ...
      <div className="relative h-1.5 w-full overflow-hidden rounded-full">
        ...
        <div className="absolute inset-0 bg-gradient-to-l from-indigo-500 via-blue-400 to-cyan-400 animate-pulse opacity-70"></div>
      </div>
    </div>
  );
}
