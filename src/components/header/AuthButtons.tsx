...
export const AuthButtons = ({
  ...
  return <div className="flex items-center gap-3">
      <Button variant="outline" onClick={handleMedicalAccess} className="flex items-center gap-2 bg-white text-primary border-primary hover:bg-primary/5 mx-0 my-[5px] text-sm text-center">
        <Building className="w-4 h-4" />
        <span>Accès professionnel</span>
      </Button>

      {user ? <Button variant="default" onClick={handleSignOut} className={navButtonClass}>
          <LogOut className="w-4 h-4 mr-1" />
          {t('logout')}
        </Button> : <Button variant="default" onClick={handleLogin} className={navButtonClass}>
          {t('login')}
        </Button>}
    </div>;
};