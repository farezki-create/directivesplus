...
import { generateSharedCode, revokeSharedCode } from "@/utils/sharedDirectivesUtils";
...
interface ShareDirectivesButtonProps {
  directiveId: string;
}

export const ShareDirectivesButton = ({ directiveId }: ShareDirectivesButtonProps) => {
  ...
  const handleGenerateCode = async () => {
    ...
    try {
      ...
      const result = await generateSharedCode(directiveId, days);
      ...
      if (result.success && result.code) {
        ...
        toast({
          variant: "destructive",
          title: "Erreur",
          description: result.error || "Impossible de générer un code d'accès"
        });
      }
    } catch (err) {
  ...
  const copyToClipboard = async () => {
    ...
    try {
      const shareUrl = `${window.location.origin}/acces-partage`;
          ...
          {!generatedCode ? (
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500">
                <strong>Lien d'accès:</strong> {window.location.origin}/acces-partage
              </p>
              
              <div className="text-xs text-gray-500 mt-2">
                {expiryDays === "1" 
                  ? "Ce code expire dans 1 jour."
                  : `Ce code expire dans ${expiryDays} jours.`
                }
              </div>
            </div>
          )}
