...
const DirectiveViewer = () => {
  ...
  const loadDirective = async () => {
    try {
      console.log("DirectiveViewer - Loading directive:", directiveId);
      ...
      setDirective(data);
      console.log("DirectiveViewer - Directive loaded:", data);
      ...
      toast({
        title: "Erreur",
        description: "Impossible de charger la directive",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleDownload = () => {
    // Créer un blob avec le contenu de la directive pour téléchargement
    const content = JSON.stringify(directive.content, null, 2);
    const blob = new Blob([content], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'directives-anticipees.json';
    ...
    toast({
      ...
      description: "Les directives anticipées sont en cours de téléchargement"
    });
  };
