...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency,
            product_data: { 
              name: donationConfig.productNames.recurring,
              description: donationConfig.descriptions.recurring
            },
            unit_amount: amount,
      ...
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/soutenir?success=true&type=subscription`,
      cancel_url: `${req.headers.get("origin")}/soutenir?canceled=true`,
    });
